...
- **Stylesheet import:** In `layout.tsx`: `import "@copilotkitnext/react/styles.css"`
- **Env file:** `.env.local`
...
- **Stylesheet import:** In `pages/_app.tsx` or `styles/globals.css`: `import "@copilotkitnext/react/styles.css"`
- **Env file:** `.env.local`
...
- **Provider placement:** Uses Angular-specific components from `@copilotkitnext/angular` (separate package)
...
- **Package:** `@copilotkitnext/angular` instead of `@copilotkitnext/react`
...
- **Stylesheet import:** In `main.tsx` or `App.tsx`: `import "@copilotkitnext/react/styles.css"`
...
- **Uses:** `@copilotkitnext/runtime` and `@copilotkitnext/agent`
- **Does NOT need:** `@copilotkitnext/react` or `@copilotkitnext/core`
- **Endpoint factories:** `createCopilotEndpointExpress` (multi-route) or `createCopilotEndpointSingleRouteExpress` (single-route), both from `@copilotkitnext/runtime/express`
- **Env file:** `.env` (loaded via `dotenv`)

### Standalone Backend (Hono)

- **Same as Express** but uses `createCopilotEndpoint` or `createCopilotEndpointSingleRoute` from `@copilotkitnext/runtime`
