...
5. **Develop** with `pnpm dev` (watches all packages) or target a specific package with `nx run @copilotkitnext/<pkg>:dev`.
6. **Write and run tests** with `nx run @copilotkitnext/<pkg>:test`. All v2 packages use Vitest.
...
| Run single package tests | `nx run @copilotkitnext/core:test` |
...
- V2 (`@copilotkitnext/*`) is the real implementation. V1 (`@copilotkit/*`) wraps V2.
